...
Template.menInShortsReport.onCreated(function() {
	this.subscribe('menInShorts');
	this.subscribe('judges');
	this.subscribe('candidates');
});

Template.menInShortsReport.helpers({
	candidates: function() {
	...
	getJudgeScore: function(judge, candidate) {
		...
		var score = MenInShorts.findOne({judgeId: judgeId, candidateId: candidate}).total;
		candidateScore += score;
		return parseFloat(score).toFixed(2);
	},
	getCandidateScore: function(candidate) {
		var score = 0;
		MenInShorts.find({candidateId: candidate}).forEach(function(e) {
