Template.formalAttire.onCreated(function() {
	this.subscribe('candidates');
	this.subscribe('formalAttiresJudge', {
		eventId: Router.current().params._id,
		judgeId: Meteor.user()._id
	});
});

Template.formalAttire.rendered = function() {
...
Template.formalAttire.events({
	...
	'submit form': function(evt, tmpl) {
		...
		Meteor.call('addFormalAttireScore', criteria, function(error) {
			if(error) {
...
Template.formalAttire.helpers({
	...
	formal: function() {
		return FormalAttires.findOne({candidateId: Session.get('selectedCandidateId')});
	}
});