Template.productionNumber.onCreated(function() {
	this.subscribe('candidates');
	this.subscribe('productionNumbersJudge', {
		eventId: Router.current().params._id,
		judgeId: Meteor.user()._id
	});
});

Template.productionNumber.rendered = function() {
...
Template.productionNumber.events({
	...
	'submit form': function(evt, tmpl) {
		...
		Meteor.call('addProductionNumberScore', criteria, function(error) {
			if(error) {
...
Template.productionNumber.helpers({
	candidates: function() {
	...
	production: function() {
		return ProductionNumbers.findOne({candidateId: Session.get('selectedCandidateId')});
	}
});