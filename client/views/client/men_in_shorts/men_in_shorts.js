Template.menInShorts.onCreated(function() {
	this.subscribe('candidates');
	this.subscribe('menInShortsJudge', {
		eventId: Router.current().params._id,
		judgeId: Meteor.user()._id
	});
});

Template.menInShorts.rendered = function() {
...
Template.menInShorts.events({
	...
	'submit form': function(evt, tmpl) {
		...
		Meteor.call('addMenInShortsScore', criteria, function(error) {
			if(error) {
...
Template.menInShorts.helpers({
	...
	men: function() {
		return MenInShorts.findOne({candidateId: Session.get('selectedCandidateId')});
	}
});