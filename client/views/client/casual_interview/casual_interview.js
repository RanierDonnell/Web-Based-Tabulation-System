Template.casualInterview.onCreated(function() {
	this.subscribe('candidates');
	this.subscribe('casualInterviewsJudge', {
		eventId: Router.current().params._id,
		judgeId: Meteor.user()._id
	});
});

Template.casualInterview.rendered = function() {
...
Template.casualInterview.events({
	...
	'submit form': function(evt, tmpl) {
		...
		Meteor.call('addCasualInterviewScore', criteria, function(error) {
			if(error) {
...
Template.casualInterview.helpers({
	candidates: function() {
	...
	casual: function() {
		return CasualInterviews.findOne({candidateId: Session.get('selectedCandidateId')});
	}
});